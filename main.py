...
from PyQt5.QtWidgets import (
    QApplication, QMainWindow, QTabWidget, QWidget, QVBoxLayout, QComboBox, 
    QPushButton, QProgressBar, QMessageBox, QHBoxLayout, QTextEdit, QLineEdit, 
    QWidgetAction, QSplitter, QLabel, QSizePolicy, QListWidget, QListWidgetItem, 
    QFrame, QDialog, QFormLayout, QDialogButtonBox, QGroupBox, QCheckBox
)
from PyQt5.QtCore import Qt, pyqtSignal, QThread, QObject, QUrl
from PyQt5.QtGui import QDesktopServices

# 自定义模块导入
from ollama_handler import get_api_handler
...
class Logger:
    ...
    def __init__(self):
        ...
        # 创建并配置日志记录器
        logger = logging.getLogger('OLLM_Note_Flow')
...
class UIHelper:
    ...
    @staticmethod
    def create_styled_button(text, color="#3498db", hover_color="#2980b9", pressed_color="#1f6aa5"):
        """创建一个带样式的按钮"""
...
class ApiConfigDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.model_info = QLabel("请输入要使用的模型名称。此名称必须与API支持的模型名称匹配。")
        self.model_info.setWordWrap(True)
        self.model_info.setStyleSheet("color: #666; font-size: 10px;")
    ...
    def test_api_connection(self):
        ...
        # 创建临时API处理器进行测试
        from ollama_handler import RemoteAPIHandler
        import json
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('OLLM Note Flow')
    ...
    def create_top_control_panel(self):
        ...
        self.install_model_btn.clicked.connect(self.install_ollama_model)
    ...
    def handle_remote_config_selection(self, selection):
        ...
        for i, config in enumerate(Config.REMOTE_API_CONFIGS):
            if config.get('name', '') == selection:
                # ConfigManager.switch_remote_config(i)  # 错误调用
                # 使用正确的方法，更新当前远程配置索引
    ...
    def offer_ollama_solutions(self):
        ...
        msg_box.setText("无法加载Ollama本地模型，请选择以下解决方案：")
    ...
    def init_ui_components(self):
        '''初始化各个功能界面组件'''
        from feature import CompletionTab, FileProcessingTab, ChatTab
        
        self.tabs.addTab(FileProcessingTab(self.api_handler), '文件处理')
        self.tabs.addTab(ChatTab(self.api_handler), '对话')
        # self.tabs.addTab(CompletionTab(self.api_handler), '文本生成')
